...
class AgentGetCompleteSolution(ScAgentClassic):
    def __init__(self):
        super().__init__("action_get_complete_solution")

    def on_event(self, event_element: ScAddr, event_edge: ScAddr, action_element: ScAddr) -> ScResult:
        result = self.run(action_element)
        is_successful = result == ScResult.OK
        finish_action_with_status(action_element, is_successful)
        self.logger.info("AgentGetCompleteSolution finished %s",
                         "successfully" if is_successful else "unsuccessfully")
        return result

    def run(self, action_node: ScAddr) -> ScResult:
        self.logger.info("AgentGetCompleteSolution started")

        try:
            problem_number_link_addr = get_action_arguments(action_node, 1)[0]

            if not problem_number_link_addr.is_valid():
                self.logger.error('AgentGetCompleteSolution: there are no argument with problem number')
            ...
            if len(links_with_problem_number) == 0:
                self.logger.error('AgentGetCompleteSolution: there are no problems with such problem number')
                return ScResult.ERROR
            ...
            if len(problem_number_links) == 0:
                self.logger.error('AgentGetCompleteSolution: there are no problem links with such problem number')
                return ScResult.ERROR
            elif len(problem_number_links) > 1:
                self.logger.error('AgentGetCompleteSolution: there are more than 1 problem with such problem number')
                return ScResult.ERROR
            ...
            results = template_search(template)
            if len(results) == 0:
                self.logger.error('AgentGetCompleteSolution: there are no text problem with such problem number')
                return ScResult.ERROR
